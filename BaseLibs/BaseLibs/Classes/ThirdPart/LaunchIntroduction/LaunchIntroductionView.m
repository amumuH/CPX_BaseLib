...
-(void)createScrollView{
    ...
    for (int i = 0; i < images.count; i ++) {
        ...
        if (i == images.count - 1) {
            //判断要不要添加button
            if (!isScrollOut) {
                _netButton = [[UIButton alloc] initWithFrame:CGRectMake(enterBtnFrame.origin.x, enterBtnFrame.origin.y, enterBtnFrame.size.width, enterBtnFrame.size.height)];
                [_netButton setImage:[UIImage imageNamed:enterBtnImage] forState:UIControlStateNormal];
                [_netButton addTarget:self action:@selector(enterBtnClick) forControlEvents:UIControlEventTouchUpInside];
                [imageView addSubview:_netButton];
