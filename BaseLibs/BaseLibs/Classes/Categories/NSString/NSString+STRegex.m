...
- (BOOL)cpx_isValidateWithRegex:(NSString *)regex
{
...
- (BOOL)cpx_isValidUrl
{
    ...
    return [self cpx_isValidateWithRegex:regex];
}

/** 邮政编码 */
- (BOOL)cpx_isValidPostalcode {
...
/** 去掉两端空格和换行符 */
- (NSString *)cpx_stringByTrimmingBlank
{
    return [self stringByTrimmingCharactersInSet:[NSCharacterSet whitespaceAndNewlineCharacterSet]];
}

/** 去掉html格式 */
- (NSString *)cpx_removeHtmlFormat;
{
...
- (BOOL)cpx_isIncludingEmoji {
    BOOL __block result = NO;
    
    [self enumerateSubstringsInRange:NSMakeRange(0, [self length])
                             options:NSStringEnumerationByComposedCharacterSequences
                          usingBlock: ^(NSString* substring, NSRange substringRange, NSRange enclosingRange, BOOL* stop) {
                              if ([substring cpx_isEmoji]) {
                                  *stop = YES;
                                  result = YES;
                              }
                          }];
